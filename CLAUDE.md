...
- **アナリティクス:** GTM + GA4（16イベント + ユーザープロパティ + コンバージョン、全てコード管理）+ MCP サーバー
...
├── components/       # React UIコンポーネント（64ファイル）
...
└── data/             # クイズデータ（JSON、732問）+ scenarios.ts
...
- **実践シナリオ（scenarioSelect）:** 実務に即したストーリー形式で既存問題を出題（22シナリオ）
- **キーワード検索（QuizSearch）:** 732問から検索、解説をその場で展開可能。10件超は全件表示画面
- **解説リーダー（ExplanationReader）:** 732問の解説をクイズなしで閲覧。カテゴリ・難易度・ステータスフィルタ + ドキュメントページ別サブフィルタ
...
| レイヤー | テスト数 | ツール | 内容 |
|---------|---------|--------|------|
| Domain | 314 | Vitest | entities, services, valueObjects |
...
```bash
bun run check         # 型 + lint + 410テスト + 732問（~5秒）
bun test              # Unit + Store（410テスト、~2秒）
