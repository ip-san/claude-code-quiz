...
- **テスト:** Vitest（399テスト）+ Playwright E2E（24テスト）
...
├── components/       # React UIコンポーネント（61ファイル）
...
# 品質チェック
bun run check         # 型チェック + lint + 399テスト + 668問チェック（一括）
bun test              # ユニット + Store テスト（399テスト、Vitest）
...
- **実践シナリオ（scenarioSelect）:** 実務に即したストーリー形式で既存問題を出題（12シナリオ）
...
| E2E | 16 | Playwright | ユーザーフロー（ウェルカム→クイズ→結果） |
| Visual Regression | 8 | Playwright | スクリーンショット比較（ライト/ダーク × 4画面） |

```bash
bun run check         # 型 + lint + 399テスト + 668問（~5秒）
bun test              # Unit + Store（399テスト、~2秒）
bun run test:e2e      # E2E + Visual（18テスト、~16秒）
```
