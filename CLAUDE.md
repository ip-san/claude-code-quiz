...
└── data/             # クイズデータ（JSON、713問）+ scenarios.ts
...
# 品質チェック
bun run check         # 型チェック + lint + 408テスト + 713問チェック（一括）
...
- **キーワード検索（QuizSearch）:** 713問から検索、解説をその場で展開可能。10件超は全件表示画面
- **解説リーダー（ExplanationReader）:** 713問の解説をクイズなしで閲覧。カテゴリ・難易度・ステータスフィルタ + ドキュメントページ別サブフィルタ
...
```bash
bun run check         # 型 + lint + 408テスト + 713問（~5秒）
