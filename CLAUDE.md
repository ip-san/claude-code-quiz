...
- **アナリティクス:** GTM + GA4（イベント計測 + エラー計測）+ MCP サーバー（Claude Code から GA4 データをクエリ）
- **品質自動化:** `/quality-loop`（GA4分析 → コードレビュー → クイズ生成 → クイズ検証の自動化ループ）
- **テスト:** Vitest（408テスト）+ Playwright E2E（47テスト）
...
| E2E | 17 | Playwright | ユーザーフロー（ウェルカム→クイズ→結果） |
| Visual Regression | 4 | Playwright | スクリーンショット比較（ライト/ダーク × 4画面） |
