...
describe('LocalStorageProgressRepository', () => {
  ...
  describe('load()', () => {
    ...
    it('migrates gs- prefixed IDs and persists the migrated data', async () => {
      // gs-001 → bp-082 per idMigration
      const storedWithGsId = buildValidProgressJson({
        questionProgress: {
          'gs-001': {
            questionId: 'gs-001',
            attempts: 1,
      ...
      localStorage.setItem(STORAGE_KEY, storedWithGsId)

      const progress = await repo.load()

      // The migrated ID should be present
      expect(progress.questionProgress['bp-082']).toBeDefined()
      expect(progress.questionProgress['gs-001']).toBeUndefined()

      // The stored value should have been updated with the migrated ID
      const stored = localStorage.getItem(STORAGE_KEY)
      expect(stored).not.toBeNull()
      expect(stored).not.toContain('"gs-001"')
      expect(stored).toContain('"bp-082"')
    })
  })
