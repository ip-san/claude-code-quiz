...
describe('SessionRepository', () => {
  ...
  describe('load()', () => {
    ...
    it('migrates gs- prefixed question IDs and updates localStorage', () => {
      // gs-001 → bp-082 per idMigration
      const sessionWithGsId = buildValidSession({
        questionIds: ['gs-001', 'mem-002'],
        currentIndex: 0,
      })
      localStorage.setItem(STORAGE_KEY, JSON.stringify(sessionWithGsId))

      const loaded = repo.load()

      expect(loaded).not.toBeNull()
      expect(loaded!.questionIds).toContain('bp-082')
      expect(loaded!.questionIds).not.toContain('gs-001')

      // localStorage should have been updated with migrated IDs
      const storedRaw = localStorage.getItem(STORAGE_KEY)
      expect(storedRaw).not.toBeNull()
      expect(storedRaw).not.toContain('"gs-001"')
      expect(storedRaw).toContain('"bp-082"')
    })
