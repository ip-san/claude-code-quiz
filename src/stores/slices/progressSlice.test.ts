...
describe('progressSlice', () => {
  ...
  describe('getFilteredQuestions', () => {
    ...
    it('filters by category', () => {
      const filtered = useQuizStore.getState().getFilteredQuestions('memory', null)
      expect(filtered.length).toBeGreaterThan(0)
      expect(filtered.every((q) => q.category === 'memory')).toBe(true)
    })
    ...
    it('filters by both category and difficulty', () => {
      const filtered = useQuizStore.getState().getFilteredQuestions('memory', 'beginner')
      expect(filtered.every((q) => q.category === 'memory' && q.difficulty === 'beginner')).toBe(true)
    })
  })
