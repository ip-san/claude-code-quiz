...
```
Presentation (components/)  →  Application (stores/)  →  Domain (domain/)
                                                              ↑
...
```
gtm/events.json（人間が編集する唯一のファイル）
  ├─→ deploy-gtm.mjs    → GTM API でタグ・トリガー・変数を同期
  ├─→ build-container.mjs → GTM インポート用 JSON を生成（API が使えない場合）
  ├─→ setup-ga4.mjs     → GA4 Admin API でカスタムディメンション・指標を登録
  └─→ ga4-server.mjs    → パラメータ型バリデーション（数値 vs 文字列）
```
...
/quality-loop（統合オーケストレーター）
  ├── /analytics-insight  — GA4 からユーザー行動データを取得・分析
  ├── /code-review        — コード品質の多層レビュー
  ├── /generate-quiz-data — 不足カテゴリの問題を自動生成
  └── /quiz-refine        — 公式ドキュメントと全問題を照合・修正
```
