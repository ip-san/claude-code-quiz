...
**PWA（GitHub Pages）** と **Electron（デスクトップ）** の 2 系統で配信する。用途に応じてそれぞれにメリットがあり、どちらかが主・従ではない。

### 背景

チームに Claude Code を広める学習ツールとして、**導入のハードルの低さ**と**実務との連携深度**の両方が重要。

### 選択肢

| 選択肢 | メリット | デメリット |
|--------|---------|-----------|
| PWA のみ | URL 共有だけで導入完了、更新も自動 | ローカルファイルやCLIにアクセスできない |
| Electron のみ | ネイティブ体験、AI連携、オフライン完全対応 | インストールが必要、配布・更新が手間 |
| **PWA + Electron** | 用途に応じて最適な体験を提供 | ビルド設定が 2 系統必要 |

### 理由

- **PWA の強み:** URL を Slack で共有するだけで全員が使い始められる。通勤中・移動中のスマホ学習に最適
- **Electron の強み:** `~/.claude/` のセッション履歴を読み取り、AI が利用パターンを分析してパーソナライズされたクイズを自動提案。自動ログ収集、ネイティブ通知、Opus/Sonnet による成長コーチングなど PWA では実現できない機能を提供
...
- **テスタビリティ** — Domain 層は React や localStorage に依存しないため、489 テスト中 314 テストが純粋なドメインテスト
...
- Rust 製で高速（`bun run check` が型チェック + lint + 489 テスト + 752 問チェックで約 5 秒）
...
752 問のクイズは Claude Code の公式ドキュメントに基づいているが、ドキュメントは頻繁に更新される。人手で全問題を定期的に検証するのは現実的でない。
