...
**https://ip-san.github.io/claude-code-quiz/** にアクセスするだけで利用可能。インストール不要。スマホ・PC 問わずどこからでも学習できます。
...
特別トリガー（条件一致時のみ自動発動）:
└── Opus → Sonnet フォールバック（$0.15/回 or $0.03/回）
    ├── 初回: あなたの学習者タイプを深く分析
...
**症状**: 「分析」ボタンを押してもエラーになる

**原因と対処:**

| 原因 | 対処 |
|------|------|
| Claude CLI が未インストール | `claude --version` で確認。未インストールなら [公式サイト](https://claude.com/claude-code) から導入 |
| API の利用制限（レート制限） | しばらく待ってから再実行。1日あたりの呼び出し頻度を下げる |
| Opus が利用できないプラン | 自動的に Sonnet にフォールバックします。機能は同等に動作します |
| Haiku が利用できない | レコメンドの意図分類がスキップされ、従来のキーワードベース推薦にフォールバックします |
| ネットワークオフライン | AI 分析は利用不可。クイズ本体はオフラインで動作します（PWA キャッシュ） |

### 学習データが消えた

**症状**: 進捗やストリークがリセットされた

**原因**: ブラウザの「データを消去」やシークレットモードで localStorage が削除された

**対処:**
- PWA 版: ブラウザの「サイトデータを消去」をしないように注意
- Desktop 版: `~/Library/Application Support/claude-code-quiz/` (macOS) のデータが残っていれば復旧可能
- 予防: 進捗ダッシュボードから「CSV エクスポート」で定期バックアップ
