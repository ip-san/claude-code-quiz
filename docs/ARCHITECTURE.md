...
│                  共有コードベース（src/）                         │
│                                                               │
│  components/  domain/  stores/  infrastructure/  data/        │
│                                                               │
├───────────────────────┬───────────────────────────────────────┤
│  PWA（メイン配信）      │  Electron（デスクトップ補助）            │
│                       │                                       │
│  vite.config.web.ts   │  vite.config.ts                       │
│  vite-plugin-pwa      │  electron/main.ts                     │
│  Service Worker       │  electron/preload.ts                  │
│  GitHub Pages         │  IPC (contextBridge)                  │
│                       │                                       │
│  → URL 共有で導入完了  │  → dmg/exe/AppImage でインストール     │
│  → SW で自動更新      │  → オフライン完全対応                   │
└───────────────────────┴───────────────────────────────────────┘
              │
    src/lib/platformAPI.ts（差分吸収）
```
...
Service Worker がアセットをキャッシュ
  ↓
2回目以降はキャッシュから即座に表示（オフライン対応）
  ↓
新しいバージョンがデプロイされると:
  1. SW が自動更新（autoUpdate）
  2. cleanupOutdatedCaches で古いキャッシュを削除
  3. controllerchange でリロードバナーを表示
```
