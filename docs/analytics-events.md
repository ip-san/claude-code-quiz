...
### カスタムディメンション（イベントスコープ）

| ディメンション名 | パラメータ名 | 用途 |
|-----------------|-------------|------|
| プラットフォーム | `platform` | PWA / Electron の利用比率 |
| クイズモード | `quiz_mode` | モード別の人気・完了率 |
| カテゴリ | `category` | カテゴリ別の学習傾向 |
| アクション | `action` | 完了/スキップ等の区分 |
| チャプター | `chapter_id` | チャプター別の進捗・離脱 |
| エラーメッセージ | `error_message` | エラー内容の分類 |
| エラーソース | `error_source` | 発生箇所の特定 |
| 問題ID | `question_id` | 問題別の正答率分析 |
| 難易度 | `difficulty` | 難易度別の傾向分析 |
...
| ディメンション名 | パラメータ名 | 用途 |
|-----------------|-------------|------|
| AI活用レベル | `mastery_level` | ユーザーセグメント分析（入門者〜牽引役） |
| 累計クイズ数 | `total_quizzes` | 利用頻度によるセグメント |

ユーザープロパティはアプリ初期化時 + クイズ完了時に `set_user_properties` イベントで設定。
...
| 総問題数 | `total_questions` | 標準 | セッション全体のサイズ |
...
| イベント | 用途 |
|---------|------|
| `quiz_complete` | クイズ完了（メインの成功指標） |
| `certificate_download` | 修了証ダウンロード（達成の証） |

ユーザー獲得レポートでコンバージョン率が自動計算される。

### プロパティ設定

| 設定 | 値 | 理由 |
|------|-----|------|
| データ保持期間 | 14ヶ月 | 長期トレンド分析（デフォルト2ヶ月では不足） |
| Enhanced Measurement | 全項目 ON | スクロール深度、離脱クリック、サイト検索、ページ遷移、フォーム操作 |

## ファネルカバレッジ

```
訪問 ──→ チュートリアル ──→ クイズ開始 ──→ 個別回答 ──→ 完了 or 離脱
 GA4      tutorial_         quiz_start     quiz_answer   quiz_complete
 自動     progress                                       quiz_quit

完了後 ──→ シェア / 修了証 / セッション復帰 / ブックマーク
           share_result      session_resume   bookmark
