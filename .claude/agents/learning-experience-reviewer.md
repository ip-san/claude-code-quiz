...
適用は別スクリプト、事実の最終確認は `quiz-verifier` が行います。
...
- `node scripts/fetch-docs.mjs --assemble <category>` で根拠ドキュメントを参照可能
- CLAUDE.md ルール厳守: 正解選択肢に wrongFeedback を付けない / ダイアグラム本文に `…` `...` を入れない / 日本語UI文言の規約

### learning（難易度・出題順・図・解説構成）
- difficulty ラベルと体感のズレは `difficulty-calibrator` の観点で慎重に（GA4 正答率があれば優先）。安易な変更はしない
- 解説の「なぜ正解か / なぜ他が誤りか」の補強、対比図（comparison/hierarchy/terminal）の追加・改善を提案
- ダイアグラム追加時は comparison.items 80字以内・完全文、`…`/`...` 禁止

### ux（画面・操作フロー）
- **report-only**。`change` は null、`uxReport` に所見と推奨対応（どのコンポーネント/フローか）を書く
- 実装修正は `/self-review`・`/code-review` に回す前提でルーティング先を明記
...
3. content/learning は必要に応じてドキュメント assemble で事実を裏取り
