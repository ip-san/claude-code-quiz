...
1. `/code-review` スキルを実行する（過去のミスパターンチェックを含む）
2. Critical / High の指摘があれば修正を適用する
3. 修正後に `npx tsc --noEmit` で型チェックを確認する
4. 追加チェック（過去の教訓）:
   - ダークモード: 新規コンポーネントで `dark:` prefix なしのダーク系クラスがないか
   - 抽象化: `grep "Claude Code" src/` でハードコード残存がないか（theme.ts 内は OK）
   - 未使用 import: `npx biome check src/ mcp/ scripts/` で検出
