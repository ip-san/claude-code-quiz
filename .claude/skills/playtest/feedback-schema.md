...
```json
{
  ...
  "sessionMode": "welcome-default | random | category:memory | review",
  "playedCount": 9,
  "items": [
    {
      "order": 1,
...
```json
{
  "resolvedAt": "<stamp>",
  "byDomain": {
    "content":  [{ "quizId": "mem-012", "persona": "beginner", ... item ... }],
    "learning": [ ... ],
    "ux":       [ ... ]
  },
  ...
  "verdicts": [
    {
      ...
      "docRef": "memory.md L55 等（content の場合必須）",
      "change": {
...
- content の `accept/modify` は事実を変えないこと。変える場合は `docRef` 必須。最終的に `quiz-verifier` が再検証する
...
`from` が現行値と不一致なら **そのверdictをスキップしてログ**（安全側）。適用後 `quiz:post-add` と `quiz-verifier` 事実ゲートを実行。
