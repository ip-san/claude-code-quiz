...
overview タグ付き問題（全体像モード）とシナリオ問題には、以下の基準を適用する:
...
`src/data/scenarios.ts` のレビュー・改善時に適用する基準。

### ナラティブの品質

各ナラティブは以下の3要素を少なくとも1つ含むこと:

1. **現場の知恵（Field Wisdom）**: 公式ドキュメントには書かれていない、経験者だけが知っている教訓
   - 良い例: 「ベテランほどこの『準備』に時間をかける」「READMEを読め、では誰も読まない」
   - 悪い例: 「MCPとは何か理解しましょう」（ドキュメントの言い換え）
2. **失敗の具体的描写**: 何を間違えるとどうなるか
   - 良い例: 「Claudeがリトライ上限を増やそうとしている。違う、パース部分が原因だ」
   - 悪い例: 「素早く操作するためのショートカットも覚えておきたい」（動機が曖昧）
3. **感情的なフック**: 読者が「自分ごと」と感じる緊迫感や達成感
   - 良い例: 「え、もうPR出せたんですか？」「正直、最初は懐疑的でした」
   - 悪い例: 「次はXXを設定しよう」（段取り説明のみ）
...
1. **導入**: 状況設定と緊張感（時刻・場所・ステークホルダー・締切など具体性を持たせる）
2. **上昇**: 各ナラティブで問題の複雑さが段階的に上がる
3. **転換**: 予期せぬ困難や発見（「修正したのにまだ動かない」「的外れな方向に進み始めた」）
4. **解決**: 具体的な成果と感情（時刻、数値、周囲の反応）

### 問題と文脈の適合

- **シナリオの難易度と問題の難易度が矛盾しないこと**: advanced シナリオに beginner-only の基礎問題は避ける
- **ナラティブの状況が問題の正解を自然に動機づけること**: 「フックで制御しよう」→ フックの問題、のように文脈が問題の学習目的と一致
- **重複は許容するが、文脈が異なること**: 同じ問題が2つのシナリオに登場してもよいが、「なぜここでこの知識が必要か」が異なるシナリオでは違う理由であること

### 問題の追加・入替の判断基準

以下に該当する場合、シナリオに新しい問題を追加（またはより適切な問題に入替）する:

- **実践的に重要なトピックが未カバー**: /doctor、worktree、画像添付、コスト確認など、実務で頻繁に使う機能
- **ストーリーの自然な発展として必要**: ナラティブの流れで「ここで○○が必要になる」場面があるのに問題がない
- **文脈ミスマッチ**: 問題の内容がナラティブの状況と噛み合っていない（例: 障害対応中に「ショートカットも覚えたい」）
- **難易度ミスマッチ**: advanced シナリオに基礎知識の問題がある

**追加しない:**
- 問題数が7問を超える場合（シナリオが長すぎると学習効果が下がる）
- ストーリーの流れを壊してまで問題を詰め込む場合
