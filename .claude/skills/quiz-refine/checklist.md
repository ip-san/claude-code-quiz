...
distractor の lint フラグ有無に関わらず、**correctIndex が指す正解そのものが現行ドキュメントで本当に正しいか**を毎回確認する。lint は「ありえない token」を持つ distractor を拾うが、**正解が静かに古くなった（doc ドリフト）問題は検出できない**。incremental スキャンでも、機能のデフォルト/仕様変更に該当する問題は必ず再評価する。
- **真の正解が選択肢に存在しないケースを最重視**（ユーザーが正しい知識で選んでも不正解になる = 最悪の体験）。実例: 正解が「ターミナルのタイトル」だが実際は「画面下部のバー」(key-031)、「中間省略」だが実際は「ファイル保存」(tool-027)、「プロジェクト設定不可」だが実際は「可（信頼ダイアログ承認後）」(mem-060)。
- **数値・件数・色数・モデル限定の変化に注意**。実例: タスク表示「最大10件→5件」、PR バッジ「5色（紫含む）→4色（紫なし・マージ後消失）」、復元オプション「5→6」、Fast mode「同一モデル→Opus専用・非Opusは自動切替」、MCP ツール定義「リクエストごと→デフォルト遅延ロード」、@import「5階層→4ホップ」、`/simplify`「4並列レビューエージェント」。
- multi-select は **correctIndices に漏れがないか**（実在の有効手段を不正解扱いしていないか。実例: `ANTHROPIC_MODEL` を除外した cmd-065）も確認。
- **severity:** 正解そのものが事実誤り/真の正解が選択肢にない = **critical**。正解は妥当だが古い挙動を含む = **major**。

### A-2. 権威ソースは個別 docs を優先
**assembled/結合ドキュメントには古い記述が残ることがある**（troubleshooting セクション等）。`docs/<page>.md`（フェッチ生ファイル）と個別ページを最終権威とし、assembled だけで判定しない。過去に assembled が「three review agents」「five hops」「2%」と古いまま、個別 docs が正典（4 agents / four hops / 1%）だった事例あり。

### A-3. 修正適用前の二重確認（エージェント指摘の検証）
並列監査エージェントの指摘は**そのまま適用せず、必ず自分で docs を再照合してから修正する**。エージェントは選択肢を途中までしか読まず誤指摘することがある（実例: key-044「4種→実は7種で正しい」、ses-126「rm は確認必要で正しい」を誤って critical 報告）。「正解が誤り」という指摘ほど偽陽性のコストが高い。
