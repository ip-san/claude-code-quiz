...
## Step 0: 機械的 lint + 構造チェック + 差分抽出

**2段階に分けて実行する。** `quiz:lint` は `quizzes.json` を変更する可能性があるため、`verify:diff` のハッシュ計算より先に完了させる必要がある。
...
**Bash ツール 3（同時に呼び出す）:**
```bash
rm -f .claude/tmp/verify_*.json .claude/tmp/verify_*.md
```
...
**Haiku サブエージェントは `.json` 形式の指示を無視して `.md` で出力することが多い（実績: 約50%）。** レポート収集時は以下の順で確認する：

1. `.claude/tmp/verify_{CATEGORY}.json` を Read → あればそのまま使用
2. `.json` がない場合、`.claude/tmp/verify_{CATEGORY}.md` を Read → .md の内容から issues を確認
3. どちらもない場合、Task の返却メッセージ（result）から直接 issues を読み取る

`.md` レポートの場合、構造化されていないため主エージェントが内容を解釈して照合する。
